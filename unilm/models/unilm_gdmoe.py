...
from torch import Tensor, layer_norm, rand
...
from unilm.models.unilm_gshard import UniLMGshardModelConfig
from unilm.modules.gdmoe import Top1Gate, Top2Gate, MOELayer
...
@dataclass
class UniLMGDMoEModelConfig(UniLMGshardModelConfig):
    # parameters for gdmoe
    tmoe_routing_dim_reduction: bool = field(default=False)
    tmoe_routing_dim: int = field(default=16)
    tmoe_routing_hard_cosine: bool = field(default=False)

    insert_transformer_moe_layers: str = field(
        default="", 
        metadata={
            "help": "Convert moe_freq(Frequency at which we insert MoE Transformer layers) into transformer_moe_layers if it is null"
        },)
    moe_sublayers: int = field(default=1)
    moe_dropout: float = field(default=0.1)
    moe_activation_dropout: float = field(default=0.1)
    capacity_factor: int = field(default=2)
    token_shuffle: bool = field(default=False)
    layer_norm_after_moe: bool = field(default=False)
    exp_level_drop: float = field(default=0.1)
    dropout_interval: int = field(default=500)
    var_coef: float = field(default=0.0)
    group_num: int = field(default=1)
    coef_type: int =field(default=1)


@register_model("unilm_gdmoe", dataclass=UniLMGDMoEModelConfig)
class UniLMGDMoEModel(UniLMModel):
    @classmethod
...
class UniLMGDMoEBody(UniLMBody):
    ...
    def __init__(
        ...
        if cfg.insert_transformer_moe_layers:
            insert_transformer_moe_layers = cfg.insert_transformer_moe_layers.split(',')
            offset = 0
            for insert_transformer_moe_layer in insert_transformer_moe_layers:
                is_moe_layer = True
                only_moe_layer = True
                self.layers.insert(int(insert_transformer_moe_layer) + offset, self.build_encoder_layer(cfg, is_moe_layer=is_moe_layer, only_moe_layer=only_moe_layer))
                offset += 1
        self.num_layers = len(self.layers)

        self.apply(init_bert_params)

        # after bert param init, re-init moe expert_centroids with orthogonal
        def init_expert_centroids_params(module):
            if hasattr(module, 'wg') and isinstance(module.wg, nn.Linear):
                torch.nn.init.orthogonal_(module.wg.weight, gain=0.1)
        self.apply(init_expert_centroids_params)

        if safe_getattr(cfg, 'rescale_init', False):
            self.rescale_fixup()
        

    def build_encoder_layer(self, cfg, is_moe_layer=False, only_moe_layer=False):
        if is_moe_layer and only_moe_layer:
            layer = UniLMBaseMoELayer(cfg)
        elif is_moe_layer:
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if isinstance(layer, UniLMMoeLayer) or isinstance(layer, UniLMBaseMoELayer):
    ...
    def rescale_fixup(self):
        ...
        for layer_id in range(len(self.layers)):
            ...
            if hasattr(layer, 'moe_layer'):
                if hasattr(layer.moe_layer.experts[0], 'fc2'):
                    for expert in layer.moe_layer.experts:
                        rescale(expert.fc2.weight.data, layer_id + 1)
                else:
                    for expert in layer.moe_layer.experts:
                        for expert_network in expert.expert_network:
                            rescale(expert_network.fc2.weight.data, layer_id + 1)
            else:
...
class UniLMBaseMoELayer(nn.Module):
    """
    Don't contain attention layer in UniLMBaseMoELayer
    """
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        ...
        if self.cfg.layer_norm_after_moe:
            self.final_layer_norm = LayerNorm(self.embed_dim)
            if safe_getattr(cfg, "ffn_layernorm", False):
                self.ffn_layer_norm = LayerNorm(cfg.encoder_ffn_embed_dim, elementwise_affine=False)
            else:
...
class MoESublayer(nn.Module):
    def __init__(self, args, embed_dim, ffn_dim):
        super().__init__()
        self.activation_fn = utils.get_activation_fn(
        ...
        self.dropout_module = FairseqDropout(
            args.moe_dropout, module_name=self.__class__.__name__
        )
        self.activation_dropout_module = FairseqDropout(
            args.moe_activation_dropout, module_name=self.__class__.__name__
        )
    ...
    def forward(self, xs):
        ...
        xs = self.activation_fn(self.fc1(xs))
        xs = self.activation_dropout_module(xs)
        xs = self.fc2(xs)
        xs = self.dropout_module(xs)
        xs = residual + xs
...
class NFeedForwardNetwork(nn.Module):
    """
        Feed Forward Network layer in the Transformer model
    """
    def __init__(self, args, embed_dim, ffn_dim, dropout_module=None, n=1):
        super().__init__()
        expert_network = []
        for i in range(n):
            expert_network.append(MoESublayer(args, embed_dim, ffn_dim))
        self.expert_network = nn.ModuleList(expert_network)
...
def make_experts(args, embed_dim, expert_ffn_dim, dropout_module, n=1) -> nn.ModuleList:
    ...
    if args.moe_expert_count >= world_size:
        ...
        for i in range(local_moe_expert_count):
            with utils.set_torch_seed(start_seed + ddp_rank * local_moe_expert_count + i):
                if n > 1:
                    expert_list.append(NFeedForwardNetwork(args, embed_dim, expert_ffn_dim, dropout_module, n))    
                else:
...
@register_model_architecture("unilm_gdmoe", "unilm_gdmoe_base")
...
@register_model_architecture("unilm_gdmoe", "unilm_gdmoe_xlarge_24L")
...
@register_model_architecture("unilm_gdmoe", "unilm_gdmoe_xlarge_24L_post")
def xlarge_24L_unilm_architecture(args):
    args.encoder_embed_dim = safe_getattr(args, "encoder_embed_dim", 2048)
    args.encoder_ffn_embed_dim = safe_getattr(args, "encoder_ffn_embed_dim", 8192)
    args.encoder_layers = safe_getattr(args, "encoder_layers", 24)
...
@register_model_architecture("unilm_gdmoe", "unilm_gdmoe_xlarge_48L")
def xlarge_24L_unilm_architecture(args):
...
@register_model_architecture("unilm_gdmoe", "unilm_gdmoe_xlarge_36L")
def xlarge_24L_unilm_architecture(args):
    args.encoder_embed_dim = safe_getattr(args, "encoder_embed_dim", 1536)
    args.encoder_ffn_embed_dim = safe_getattr(args, "encoder_ffn_embed_dim", 6144)
...
@register_model_architecture("unilm_gdmoe", "unilm_gdmoe_xsmall")
def xsmall_unilm_architecture(args):
    args.encoder_embed_dim = safe_getattr(args, "encoder_embed_dim", 384)
    args.encoder_ffn_embed_dim = safe_getattr(args, "encoder_ffn_embed_dim", 1536)
