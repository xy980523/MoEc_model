...
@register_criterion(
    "label_smoothed_cross_entropy_moe", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        lm_loss = loss
        loss = lm_loss + self.cfg.moe_gate_loss_wt * gate_loss

        '''
        # Avg
        loss = loss / sample_size
        lm_loss = lm_loss / sample_size
        gate_loss = gate_loss / sample_size
        ...
        logging_output = {
            ...
            "moe_loss": gate_loss.data if type(gate_loss) is not float else 0,
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "lm_loss", sum(log.get("lm_loss", 0) for log in logging_outputs) / sample_size / math.log(2), sample_size, round=3
        )
        metrics.log_scalar(
            "moe_loss", sum(log.get("moe_loss", 0) for log in logging_outputs) / sample_size / math.log(2), sample_size, round=3
        )
        metrics.log_scalar(
