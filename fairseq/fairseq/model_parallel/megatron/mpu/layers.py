...
def _initialize_affine_bias(bias, output_size, per_partition_size,init_method, stride=1):
    """Initialize affine weight for model parallel.
    ...
    # Initialize master weight
    master_bias = torch.empty(output_size, dtype=bias.dtype, requires_grad=False)
    init_method(master_bias)

    # Split and copy
    per_partition_per_stride_size = divide(per_partition_size, stride)
    bias_list = torch.split(master_bias, per_partition_per_stride_size, dim=0)
    rank = get_model_parallel_rank()
    my_bias_list = bias_list[rank::world_size]

    with torch.no_grad():
        torch.cat(my_bias_list, dim=0, out=bias)
...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, bias=True, gather_output=True,
                 ...
                 keep_master_weight_for_test=False, init_method_bias=None):
        ...
        if bias:
            ...
            if init_method_bias is None:
                # Always initialize bias to zero.
                with torch.no_grad():
                    self.bias.zero_()
            else:
                _initialize_affine_bias(
                    self.bias, self.output_size, self.output_size_per_partition, init_method_bias,
                    stride=stride
                )
        else:
